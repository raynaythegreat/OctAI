...
4. Nhập Token và EncodingAESKey vào file cấu hình AI Business HQ, khởi động dịch vụ rồi quay lại bảng điều khiển quản trị để lưu (WeCom sẽ gửi yêu cầu xác minh)
...
Người dùng gửi tin nhắn
  │
  ▼
AI Business HQ trả về ngay {finish: false} (Agent bắt đầu xử lý)
  │
  ▼
...
Nếu thời gian xử lý của agent vượt quá khoảng 30 giây (cửa sổ polling tối đa của WeCom là 6 phút), AI Business HQ sẽ:
...
Khi `welcome_message` được cấu hình, AI Business HQ sẽ tự động phản hồi bằng tin nhắn đó khi người dùng mở cửa sổ chat với AI Bot (sự kiện `enter_chat`). Để trống để bỏ qua im lặng.
...
- Kiểm tra log AI Business HQ xem có nhận được yêu cầu GET từ WeCom không
