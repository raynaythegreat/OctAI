...
La configuration des outils de OctAi se trouve dans le champ `tools` de `config.json`.
...
Par défaut, OctAi bloque les commandes dangereuses suivantes :
...
Le garde exec ne valide que la commande de niveau supérieur envoyée à OctAi. Il n'inspecte **pas** récursivement les processus enfants générés par les outils de build ou les scripts après le démarrage de cette commande.
...
Toutes les options de configuration peuvent être remplacées via des variables d'environnement au format `OCTAI_TOOLS_<SECTION>_<KEY>` :
