...
| 変数              | 説明                                                                                                                             | デフォルトパス            |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `PICOCLAW_CONFIG` | 設定ファイルのパスを上書きします。aibhq がどの `config.json` を読み込むかを直接指定し、他のすべての場所を無視します。 | `~/.aibhq/config.json` |
| `PICOCLAW_HOME`   | aibhq データのルートディレクトリを上書きします。`workspace` やその他のデータディレクトリのデフォルト場所を変更します。          | `~/.aibhq`             |
...
AI Business HQ は設定されたワークスペース（デフォルト: `~/.aibhq/workspace`）にデータを保存します：
...
AI Business HQ はデフォルトでサンドボックス環境で実行されます。Agent は設定されたワークスペース内のファイルアクセスとコマンド実行のみが可能です。
...
exec セキュリティガードは AI Business HQ が直接起動するコマンドラインのみを検査します。`make`、`go run`、`cargo`、`npm run`、またはカスタムビルドスクリプトなどの開発ツールが生成する子プロセスは再帰的に検査しません。
...
* ビルトインガードより強力な分離が必要な場合は、コンテナまたは VM 内で AI Business HQ を実行してください。
...
AI Business HQ は定期タスクを自動実行できます。ワークスペースに `HEARTBEAT.md` ファイルを作成してください：
...
| `volcengine` | LLM（Volcengine 直接）                  | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=AI Business HQ&utm_content=AI Business HQ&utm_medium=devrel&utm_source=OWO&utm_term=AI Business HQ) |
...
> **新機能：** AI Business HQ は**モデル中心**の設定アプローチを採用しました。`vendor/model` 形式（例：`zhipu/glm-4.7`）を指定するだけで新しい Provider を追加できます — **コード変更不要！**
...
| **VolcEngine (Doubao)** | `volcengine/`          | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI     | [取得](https://www.volcengine.com/activity/codingplan?utm_campaign=AI Business HQ&utm_content=AI Business HQ&utm_medium=devrel&utm_source=OWO&utm_term=AI Business HQ) |
| **Antigravity**         | `antigravity/`         | Google Cloud                                        | Custom     | OAuth のみ                                                       |

#### ロードバランシング

同じモデル名に複数のエンドポイントを設定すると、AI Business HQ が自動的にラウンドロビンします：
...
AI Business HQ はプロトコルファミリーで Provider をルーティングします：
...
AI Business HQ は `cron` ツールを通じて cron スタイルのスケジュールタスクをサポートします。
