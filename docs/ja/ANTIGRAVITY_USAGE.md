...
# AI Business HQ で Antigravity プロバイダーを使用する

このガイドでは、AI Business HQ で **Antigravity**（Google Cloud Code Assist）プロバイダーをセットアップして使用する方法を説明します。
...
6.  **AI Business HQ が待機しているターミナルにそれを貼り付けます**。

AI Business HQ が自動的に認証コードを抽出し、プロセスを完了します。
...
### モデルの切り替え
`~/.aibhq/config.json` でデフォルトモデルを変更するか、CLI でオーバーライドできます：
...
*   **429 レート制限**：Antigravity には厳格なクォータがあります。制限に達した場合、AI Business HQ はエラーメッセージに「リセット時間」を表示します。
*   **404 Not Found**：`aibhq auth models` リストのモデル ID を使用していることを確認してください。フルパスではなく、短い ID（例：`gemini-3-flash`）を使用してください。
