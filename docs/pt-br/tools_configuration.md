...
A configuração de ferramentas do OctAi está localizada no campo `tools` do `config.json`.
...
Por padrão, o OctAi bloqueia os seguintes comandos perigosos:
...
O guarda exec apenas valida o comando de nível superior enviado ao OctAi. Ele **não** inspeciona recursivamente processos filhos gerados por ferramentas de build ou scripts após o início desse comando.
...
Todas as opções de configuração podem ser substituídas via variáveis de ambiente com o formato `OCTAI_TOOLS_<SECTION>_<KEY>`:
