...
Chạy `octai onboard` — nó yêu cầu nhập cụm mật khẩu và tạo khóa SSH,
...
| `OCTAI_SSH_KEY_PATH` | Không | Đường dẫn đến khóa riêng SSH. Nếu không đặt, tự động phát hiện từ `~/.ssh/octai_ed25519.key` |
...
Chạy `octai onboard` để tạo tự động.

`os.UserHomeDir()` được sử dụng để phân giải thư mục home đa nền tảng (đọc `USERPROFILE` trên Windows, `HOME` trên Unix/macOS).

> **Lưu ý:** Tệp khóa SSH là bắt buộc cho mã hóa thông tin xác thực. Nếu không tìm thấy khóa và `OCTAI_SSH_KEY_PATH` không được đặt, mã hóa/giải mã sẽ thất bại. Chạy `octai onboard` để tạo khóa tự động.
...
- **Cả cụm mật khẩu và khóa SSH đều bắt buộc.** Khóa SSH đóng vai trò yếu tố thứ hai — không có nó, mã hóa/giải mã sẽ thất bại. Chạy `octai onboard` để tạo khóa nếu chưa tồn tại.
...
- **Định dạng `enc://` được quản lý phiên bản** thông qua trường `info` của HKDF (`octai-credential-v1`), cho phép nâng cấp thuật toán trong tương lai mà không làm hỏng các giá trị đã mã hóa hiện có.
