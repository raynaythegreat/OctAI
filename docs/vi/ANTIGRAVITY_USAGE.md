...
### Chuyển đổi mô hình
Bạn có thể thay đổi mô hình mặc định trong `~/.octai/config.json` hoặc ghi đè qua CLI:
...
*   **404 Không tìm thấy**: Đảm bảo bạn đang sử dụng ID mô hình từ danh sách `octai auth models`. Sử dụng ID ngắn (ví dụ: `gemini-3-flash`) thay vì đường dẫn đầy đủ.
