...
9. [Cấu hình](#cấu-hình)
10. [Tạo nhà cung cấp mới trong OctAi](#tạo-nhà-cung-cấp-mới-trong-octai)
...
Hồ sơ xác thực được lưu trữ trong `~/.octai/auth.json`:
...
Nếu nhà cung cấp của bạn yêu cầu OAuth hoặc xác thực đặc biệt, thêm case vào `cmd/octai/internal/auth/helpers.go`:
...
- **File nguồn:**
  ...
  - `pkg/auth/store.go` - Lưu trữ thông tin xác thực (`~/.octai/auth.json`)
  ...
  - `cmd/octai/internal/auth/helpers.go` - Lệnh CLI xác thực
...
- Làm mới token OAuth: `octai auth login --provider antigravity`
...
- Kiểm tra lưu trữ hồ sơ xác thực: `~/.octai/auth.json`
- Chạy lại `octai auth login --provider antigravity`
