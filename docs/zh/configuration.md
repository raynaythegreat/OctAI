...
| 变量              | 描述                                                                                                                             | 默认路径                  |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `PICOCLAW_CONFIG` | 覆盖配置文件的路径。这直接告诉 aibhq 加载哪个 `config.json`，忽略所有其他位置。 | `~/.aibhq/config.json` |
| `PICOCLAW_HOME`   | 覆盖 aibhq 数据根目录。这会更改 `workspace` 和其他数据目录的默认位置。          | `~/.aibhq`             |
...
AI Business HQ 将数据存储在您配置的工作区中（默认：`~/.aibhq/workspace`）：
...
AI Business HQ 默认在沙箱环境中运行。Agent 只能访问配置的工作区内的文件和执行命令。
...
exec 安全守卫仅检查 AI Business HQ 直接启动的命令行。它不会递归检查由 `make`、`go run`、`cargo`、`npm run` 或自定义构建脚本等开发工具产生的子进程。
...
* 如果需要比内置守卫更强的隔离，请在容器或虚拟机中运行 AI Business HQ。
...
AI Business HQ 可以自动执行周期性任务。在工作区创建 `HEARTBEAT.md` 文件：
...
> **新特性：** AI Business HQ 现在采用**以模型为中心**的配置方式。只需指定 `vendor/model` 格式（例如 `zhipu/glm-4.7`）即可接入新提供商——**无需修改任何代码！**
...
| **火山引擎 (豆包)**     | `volcengine/`     | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI    | [获取](https://www.volcengine.com/activity/codingplan?utm_campaign=AI Business HQ&utm_content=AI Business HQ&utm_medium=devrel&utm_source=OWO&utm_term=AI Business HQ) |
...
AI Business HQ 只剥离最外层的 `litellm/` 前缀再发送请求，因此 `litellm/lite-gpt4` 发送 `lite-gpt4`，而 `litellm/openai/gpt-4o` 发送 `openai/gpt-4o`。

</details>

#### 负载均衡

为同一模型名称配置多个端点，AI Business HQ 会自动轮询：
...
AI Business HQ 通过 `cron` 工具支持 cron 风格的定时任务。Agent 可以设置、列出和取消在指定时间触发的提醒或周期性任务。
