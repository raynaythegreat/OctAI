...
配置文件路径: `~/.aibhq/config.json`

### 环境变量

你可以使用环境变量覆盖默认路径。这对于便携安装、容器化部署或将 aibhq 作为系统服务运行非常有用。这些变量是独立的，控制不同的路径。

| 变量              | 描述                                                                                                                             | 默认路径                  |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `OCTAI_CONFIG` | 覆盖配置文件的路径。这直接告诉 aibhq 加载哪个 `config.json`，忽略所有其他位置。 | `~/.aibhq/config.json` |
| `OCTAI_HOME`   | 覆盖 aibhq 数据根目录。这会更改 `workspace` 和其他数据目录的默认位置。          | `~/.aibhq`             |
...
# 在 /opt/aibhq 中存储所有数据运行 aibhq
# 配置将从默认的 ~/.aibhq/config.json 加载
# 工作区将在 /opt/aibhq/workspace 创建
...
OctAi 将数据存储在您配置的工作区中（默认：`~/.aibhq/workspace`）：
...
| 选项                    | 默认值                  | 描述                          |
| ----------------------- | ----------------------- | ----------------------------- |
| `workspace`             | `~/.aibhq/workspace` | Agent 的工作目录              |
...
> 运行 `aibhq auth login --provider anthropic` 粘贴 API Token。
...
定时任务在重启后持久保存，存储于 `~/.aibhq/workspace/cron/`。
