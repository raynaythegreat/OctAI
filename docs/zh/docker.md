...
`launcher` 镜像包含所有三个二进制文件（`octai`、`octai-launcher`、`octai-launcher-tui`），默认启动 Web 控制台，提供基于浏览器的配置和聊天界面。
...
```bash
# 提问
docker compose -f docker/docker-compose.yml run --rm octai-agent -m "2+2 等于几？"
...
**2. 配置 (Configure)** (`~/.octai/config.json`)
