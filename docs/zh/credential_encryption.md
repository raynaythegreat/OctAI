...
AI Business HQ 支持对 `model_list` 配置条目中的 `api_key` 值进行加密。
...
| `PICOCLAW_SSH_KEY_PATH` | 否 | SSH 私钥路径。如未设置，自动从 `~/.ssh/aibhq_ed25519.key` 检测 |

### SSH 密钥自动检测

如果未设置 `PICOCLAW_SSH_KEY_PATH`，AI Business HQ 会查找专用密钥：
...
> **注意：** SSH 密钥文件是凭据加密的必要条件。如果未找到密钥且未设置 `PICOCLAW_SSH_KEY_PATH`，加密/解密将失败。运行 `aibhq onboard` 可自动生成密钥。
...
- **SSH 密钥在运行时为只读。** AI Business HQ 不会写入或修改 SSH 密钥文件。
