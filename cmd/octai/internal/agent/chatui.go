...
import (
	...
	"github.com/raynaythegreat/ai-business-hq/cmd/octai/internal"
	pkgagent "github.com/raynaythegreat/ai-business-hq/pkg/agent"
	pkgconfig "github.com/raynaythegreat/ai-business-hq/pkg/config"
...
func (c *chatUI) toggleMode() {
	...
	c.updateHeader()
	c.setTemporaryStatus(fmt.Sprintf("[#A855F7]Switched to %s mode[-]", c.mode.String()), 2*time.Second)
}
		...
		case tcell.KeyTab:
			...
			if strings.TrimSpace(c.input.GetText()) == "" {
				c.app.QueueUpdateDraw(func() {
					c.toggleMode()
					c.input.SetFieldBackgroundColor(tcell.NewHexColor(0x1E0F3D))
...
func (c *chatUI) handleSlashCommand(raw string) {
	...
	if skill := strings.TrimPrefix(cmd, "/"); c.skillNames[skill] {
		// Trigger skill indicator
		c.setTemporaryStatus(fmt.Sprintf("[#A855F7]%s[-] [#7B6F8E]executing...[-]", skill), 3*time.Second)
		if len(parts) > 1 {
...
func (c *chatUI) sendToLoop(text string) {
	...
	go func() {
		...
		c.app.QueueUpdateDraw(func() {
			...
			c.app.SetFocus(c.input) // Ensure focus returns to input
		})
	}()
}
...
func (c *chatUI) showModelPicker() {
	...
	list.SetTitle(" [#A855F7::b]MODEL PICKER[-]  [#7B6F8E]↑↓ select · Enter switch · Esc close[-] ")
	...
	for {
		...
		case <-ticker.C:
			...
			if tempStatus != "" {
				...
				c.app.QueueUpdateDraw(func() { c.statusLine.SetText("  [#7B6F8E]idle[-]") })
			}
		}
	}
}
