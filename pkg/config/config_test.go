...
// OCTAI_SSH_KEY_PATH to its path for the duration of the test. This is required
// whenever a test exercises encryption/decryption via credential.Encrypt or SaveConfig.
func mustSetupSSHKey(t *testing.T) {
	t.Helper()
	keyPath := filepath.Join(t.TempDir(), "aibhq_ed25519.key")
	if err := credential.GenerateSSHKey(keyPath); err != nil {
		t.Fatalf("mustSetupSSHKey: %v", err)
	}
	t.Setenv("OCTAI_SSH_KEY_PATH", keyPath)
}
...
func TestDefaultConfig_WorkspacePath_Default(t *testing.T) {
	...
	cfg := DefaultConfig()
	want := filepath.Join(fakeHome, ".octai", "workspace")
...
`
	...
	t.Setenv("OCTAI_KEY_PASSPHRASE", "test-passphrase")
...
// unchanged when OCTAI_KEY_PASSPHRASE is not set.
...
// TestLoadConfig_MixedKeys_NoPassphrase verifies that when OCTAI_KEY_PASSPHRASE
